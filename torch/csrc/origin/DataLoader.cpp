...
{                                                                             \
  ...
  sa.sa_flags = 0;                                                            \
  if (sigemptyset(&sa.sa_mask) != 0 || sigaction(SIGNAL, &sa, 0) != 0) {   \
    _exit(EXIT_FAILURE);                                                      \
  } else {                                                                    \
    raise(SIGNAL);                                                            \
  }                                                                           \
...
static void handler_SIGTERM(int sig, siginfo_t *info, void *ctx)
{
  ...
  sa.sa_handler = SIG_DFL;
  sa.sa_flags = 0;
  if (sigemptyset(&sa.sa_mask) != 0 || sigaction(SIGTERM, &sa, 0) != 0) {
    _exit(EXIT_FAILURE);
  } else {
    raise(SIGTERM);
  }
...
static PyObject *THPModule_setWorkerSignalHandlers(PyObject *module, PyObject *arg) {
  HANDLE_TH_ERRORS
  setSignalHandler(SIGBUS, &handler_SIGBUS, 0);
  setSignalHandler(SIGSEGV, &handler_SIGSEGV, 0);
  setSignalHandler(SIGTERM, &handler_SIGTERM, 0);
  setSignalHandler(SIGFPE, &handler_SIGFPE, 0);
  Py_RETURN_NONE;
...
PyMethodDef DataLoaderMethods[] = {
  {"_set_worker_signal_handlers",  (PyCFunction)THPModule_setWorkerSignalHandlers,  METH_NOARGS,   0},
  {"_set_worker_pids",             (PyCFunction)THPModule_setWorkerPIDs,            METH_VARARGS,  0},
  {"_remove_worker_pids",          (PyCFunction)THPModule_removeWorkerPIDs,         METH_O,        0},
  {"_error_if_any_worker_fails",   (PyCFunction)THPModule_errorIfAnyWorkerFails,    METH_NOARGS,   0},
  {0, 0, 0, 0}
};
